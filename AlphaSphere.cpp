...
void CAlphaSphere::Activate(float radius, CVector3f startColour, CVector3f endColour, bool zap, float zapLevel)
{
	m_isActive = true;
	m_elapsedTime = 0.0f;
	m_totalTime = 3.0f; // run the effect for three seconds -- could pass this in instead
...
void CAlphaSphere::Render()
{
	...
	glPushMatrix();
		...
		glColor4f(m_colour.x, m_colour.y, m_colour.z, 0.25);
		...
		if (m_zap) {
			...
			// Set the colour
			glColor4f(m_colour.x, m_colour.y, m_colour.z, 0.75);
			glLineWidth(2.0f);
