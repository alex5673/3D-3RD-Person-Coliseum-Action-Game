...
{
	// float yValue = m_position.y; // record y position in order to keep it the same after update of movement
	Movement(dt);
	// m_position.y = yValue; // return y position to value before update of movement so that it always stays the same height
	SetViewByMouse();
}

// Update camera rail position to move along a specific axis
void Camera::UpdateRail(double dt)
{
	m_position.z += m_speed * dt;
	m_viewPoint.z += m_speed * dt;
	MovementRail(dt);
...
// Move the rail camera based on arrow keys or WSAD keys
